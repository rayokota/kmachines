 ...
 */
final class KMachineCoordinator extends AbstractCoordinator implements Closeable {
    private static final Logger LOG = LoggerFactory.getLogger(KMachineCoordinator.class);

    public static final String KDB_SUBPROTOCOL_V0 = "v0";

    private final KMachineIdentity identity;
    private KMachineProtocol.Assignment assignmentSnapshot;
    private final KMachineRebalanceListener listener;

    /**
     * Initialize the coordination manager.
     */
    public KMachineCoordinator(
        ...
        Time time,
        long retryBackoffMs,
        KMachineIdentity identity,
        KMachineRebalanceListener listener) {
        super(
    ...
    public JoinGroupRequestData.JoinGroupRequestProtocolCollection metadata() {
        ByteBuffer metadata = KMachineProtocol.serializeMetadata(identity);
    ...
    protected void onJoinComplete(
        ...
        assignmentSnapshot = KMachineProtocol.deserializeAssignment(memberAssignment);
    ...
    protected Map<String, ByteBuffer> performAssignment(
        ...
        List<KMachineIdentity> members = new ArrayList<>();
        Map<String, KMachineIdentity> memberConfigs = new HashMap<>();
        for (JoinGroupResponseData.JoinGroupResponseMember entry : allMemberMetadata) {
            KMachineIdentity identity
                = KMachineProtocol.deserializeMetadata(ByteBuffer.wrap(entry.metadata()));
        ...
        KMachineIdentity leaderIdentity = null;
        String leaderKafkaId = null;
        Set<String> urls = new HashSet<>();
        for (Map.Entry<String, KMachineIdentity> entry : memberConfigs.entrySet()) {
            String kafkaMemberId = entry.getKey();
            KMachineIdentity memberIdentity = entry.getValue();
        ...
        short error = KMachineProtocol.Assignment.NO_ERROR;
        ...
        if (urls.size() != memberConfigs.size()) {
            ...
            error = KMachineProtocol.Assignment.DUPLICATE_URLS;
        }

        // All members currently receive the same assignment information since it is just the leader ID
        Map<String, ByteBuffer> groupAssignment = new HashMap<>();
        KMachineProtocol.Assignment assignment = new KMachineProtocol.Assignment(error, leaderKafkaId, leaderIdentity, members);
        LOG.debug("Assignment: {}", assignment);
        for (String member : memberConfigs.keySet()) {
            groupAssignment.put(member, KMachineProtocol.serializeAssignment(assignment));
        }
